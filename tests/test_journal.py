...
class TestAddEntry:
    def test_basic_add(self):
        entry = journal.add_entry("Sold 200 TSLA at $185")
        assert entry["id"] == 1
        assert "TSLA" in entry["text"]
        assert entry["ts"]
    ...
    def test_symbol_extraction(self):
        entry = journal.add_entry("Trimmed AAPL and MSFT for margin relief")
        assert "AAPL" in entry["symbols"]
        assert "MSFT" in entry["symbols"]

    def test_stopwords_excluded(self):
        entry = journal.add_entry("I SELL MY TSLA FOR A GOOD PRICE")
        assert "TSLA" in entry["symbols"]
...
class TestSearchEntries:
    def test_search_finds_match(self):
        journal.add_entry("Bought AAPL at $118")
        journal.add_entry("Sold TSLA at $185")
        results = journal.search_entries("AAPL")
...
class TestSymbolExtraction:
    ...
    def test_no_duplicates(self):
        syms = journal._extract_symbols("AAPL is great, AAPL is king")
        assert syms.count("AAPL") == 1

    def test_mixed_text(self):
        syms = journal._extract_symbols("I think TSLA will hit $250 by Q3")
        assert "TSLA" in syms
