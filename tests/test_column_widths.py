...
class TestSurprisesColumns:
    ...
    def test_data_rows_identical_width(self):
        ...
        sym_rows = [r for r in rows if any(s in r for s in ("AAPL", "TSLA", "MSFT"))]
    ...
    def test_sign_column_isolated_from_digits(self):
        ...
        # across positive and negative EPS rows. AAPL has +$0.92, MSFT has -$0.09.
        aapl = next(r for r in rows if "AAPL" in r)
        msft   = next(r for r in rows if "MSFT"   in r)
        ...
        assert aapl.index("0.92") == msft.index("0.09"), (
            f"EPS digit column drifted: AAPL col {aapl.index('0.92')}, "
            f"MSFT col {msft.index('0.09')}\n{aapl}\n{msft}"
        )
...
class TestValuationColumns:
    ...
    def test_negative_pct_no_arrow_overflow(self):
        ...
        infos, syms = self._sample()
        result = format_valuation(infos, syms)
        # TSLA and MSFT are both down in the sample. Neither should contain the
...
class TestConsolidatedColumns:
    SAMPLE = (
        ...
        "|---------|-----|---------|-------------|--------|\n"
        "| U1234567 | $500,000.00 CAD | 1.85x | 54.0% | 15.2% |\n"
        "| U9999999 | $200,000.00 CAD | 1.20x | 22.0% | 35.0% |\n"
        "\n"
        ...
        "|---------|--------|--------|-------|-----------|-----|\n"
        "| U1234567 | AAPL | 500 | $62,500.00 USD | $89,375.00 CAD | +$12,500.00 CAD |\n"
        "| U1234567 | TSLA | 200 | $40,000.00 USD | $57,200.00 CAD | -$3,000.00 CAD |\n"
        "| U9999999 | MSFT   | 100 | $9,825.00 USD  | $14,050.00 CAD |    -$425.00 CAD |\n"
        "\n"
...
class TestBriefingMoversColumns:
    def _sample(self) -> dict:
        return {
            ...
            "movers": {
                "gainers": [
                    {"symbol": "AAPL", "price": 137.50, "pct": 1.85},
                    {"symbol": "TSLA", "price": 1650.00, "pct": 2.34},
                    {"symbol": "GOOGL", "price": 180.00, "pct": 0.50},
                ],
                "losers": [
                    {"symbol": "MSFT",   "price": 98.25, "pct": -3.10},
                    {"symbol": "TSLA", "price": 412.00, "pct": -12.40},
                    {"symbol": "AMZN", "price": 875.00, "pct": -0.80},
                ],
            },
            "sectors": [],
            "news": [],
