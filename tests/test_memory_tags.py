...
class TestMemorySaveTag:
    def test_basic_save(self):
        text = "Done. [MEMORY: MSFT reports June 25] I've noted that."
        match = _RE_MEMORY_SAVE.search(text)
        assert match
        assert match.group(1) == "MSFT reports June 25"

    def test_save_with_extra_spaces(self):
        match = _RE_MEMORY_SAVE.search("[MEMORY:   TSLA target $250  ]")
        assert match
        assert match.group(1).strip() == "TSLA target $250"
