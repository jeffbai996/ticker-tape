...
class TestEarningsSurprises:
    def test_upsert_and_query(self, tmp_db):
        import db
        db.upsert_earnings("AAPL", date(2026, 2, 26), 0.89, 0.92, 3.37, 2.1)
        records = db.get_earnings_history(["AAPL"])
    ...
    def test_upsert_dedup(self, tmp_db):
        import db
        db.upsert_earnings("AAPL", date(2026, 2, 26), 0.89, 0.92)
        db.upsert_earnings("AAPL", date(2026, 2, 26), 0.89, 0.92)  # same date
        records = db.get_earnings_history(["AAPL"])
    ...
    def test_earnings_summary(self, tmp_db):
        import db
        db.upsert_earnings("AAPL", date(2026, 2, 26), 0.89, 0.92, 3.37, 2.1)
        db.upsert_earnings("AAPL", date(2025, 11, 20), 0.80, 0.85, 6.25, -1.5)
        summary = db.get_earnings_summary("AAPL")
    ...
    def test_filter_by_symbol(self, tmp_db):
        import db
        db.upsert_earnings("AAPL", date(2026, 2, 26), 0.89, 0.92)
        db.upsert_earnings("TSLA", date(2026, 3, 10), 1.50, 1.55)
        aapl = db.get_earnings_history(["AAPL"])
        assert len(aapl) == 1
...
class TestSmartAlerts:
    def test_add_typed_alert(self, alerts_file):
        ...
        a = add_alert("AAPL", ">", 70, alert_type="rsi")
        assert a["type"] == "rsi"
        alerts = load_alerts()
    ...
    def test_backward_compat_no_type_field(self, alerts_file):
        ...
        save_alerts([{"id": 1, "symbol": "AAPL", "operator": ">", "value": 100}])
    ...
    def test_evaluate_alerts_skips_non_price(self, alerts_file):
        ...
        add_alert("AAPL", ">", 70, alert_type="rsi")
        add_alert("AAPL", ">", 100, alert_type="price")
        quotes = [{"symbol": "AAPL", "price": 150.0}]
        triggered = evaluate_alerts(quotes)
    ...
    def test_evaluate_technical_rsi(self, alerts_file):
        ...
        add_alert("AAPL", ">", 70, alert_type="rsi")
        batch = {"AAPL": {"rsi": 75.0, "current": 160.0}}
    ...
    def test_evaluate_technical_sma_cross(self, alerts_file):
        from data import add_alert, evaluate_technical_alerts
        add_alert("AAPL", ">", 200, alert_type="sma_cross")
        batch = {"AAPL": {"rsi": 55.0, "current": 160.0, "sma_200": 155.0}}
        triggered = evaluate_technical_alerts(batch)
        assert len(triggered) == 1
    ...
    def test_evaluate_technical_volume(self, alerts_file):
        from data import add_alert, evaluate_technical_alerts
        add_alert("AAPL", ">", 2.0, alert_type="volume")
        batch = {"AAPL": {"vol_ratio": 2.5}}
        triggered = evaluate_technical_alerts(batch)
        assert len(triggered) == 1
    ...
    def test_evaluate_technical_missing_symbol(self, alerts_file):
        from data import add_alert, evaluate_technical_alerts
        add_alert("AAPL", ">", 70, alert_type="rsi")
        batch = {"TSLA": {"rsi": 75.0}}  # AAPL not in batch
        triggered = evaluate_technical_alerts(batch)
        assert len(triggered) == 0
...
class TestAlertsFormatter:
    ...
    def test_typed_alert_display(self):
        from screens.alerts import format_alerts
        alerts = [
            {"id": 1, "symbol": "AAPL", "type": "rsi", "operator": ">", "value": 70, "created": "2026-04-13"},
            {"id": 2, "symbol": "TSLA", "type": "sma_cross", "operator": ">", "value": 200, "created": "2026-04-13"},
...
class TestSizingFormatter:
    def test_no_ibkr_data(self):
        ...
        result = format_sizing(None, None, None, "AAPL", 100)
        assert "AAPL" in result
    ...
    def test_with_whatif_data(self):
        ...
        result = format_sizing(whatif, None, None, "AAPL", 100)
        assert "AAPL" in result
...
class TestBriefingFormatter:
    ...
    def test_with_portfolio(self):
        ...
        data = {
            ...
            "movers": {
                "gainers": [{"symbol": "AAPL", "pct": 3.5, "price": 160.0}],
                "losers": [{"symbol": "MSFT", "pct": -2.1, "price": 95.0}],
            },
            "earnings_week": [{"symbol": "TSLA", "date": "2026-04-15", "days_until": 2}],
